...
def _setup_console_encoding() -> None:
    """尽量避免 Windows 默认编码导致的 UnicodeEncodeError。"""
    try:
    ...
    def _run_ai_analysis(
        ...
        if schedule.once_analyze and schedule.period_key:
            ...
            if scheduler.already_executed(schedule.period_key, "analyze", date_str):
                period_name = schedule.period_name or schedule.period_key
                msg = f"时间段 {period_name} 今天已分析过，沿用上一次结果，未重新分析"
                print(f"[AI] 调度器: {msg}")
                return AIAnalysisResult(
                    success=False,
                    skipped=True,
...
示例:
  ...
  python -m trendradar --assistant-web    # 启动助理 Web 服务
"""
    )
    parser.add_argument(
        "--show-schedule",
    ...
    parser.add_argument(
        "--assistant-web-host",
        default="127.0.0.1",
        help="助理 Web 服务监听地址（默认: 127.0.0.1）"
    )
    parser.add_argument(
        "--assistant-web-port",
        type=int,
        default=8765,
        help="助理 Web 服务端口（默认: 8765）"
    )
    parser.add_argument(
        "--assistant-web-no-open",
        action="store_true",
        help="启动助理 Web 服务时不自动打开浏览器"
    )
    ...
    try:
        ...
        if args.assistant_web:
            if not args.assistant_web_no_open:
                webbrowser.open(
                    f"http://{args.assistant_web_host}:{args.assistant_web_port}/assistant"
                )
            run_assistant_web(
                config=config,
                host=args.assistant_web_host,
                port=args.assistant_web_port,
            )
            return

        # 常规运行时自动启动助理 Web 服务（不阻塞主流程）
        # 若端口占用或启动失败，仅提示，不影响抓取/分析流程
        if not start_assistant_web_background(config=config, host="127.0.0.1", port=8765):
            print("[助手] 自动启动失败（不影响主流程），可手动执行 --assistant-web")
