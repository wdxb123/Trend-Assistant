...
def _load_assistant_router_config(config_data: Dict) -> Dict:
    """加载助理路由配置（词典 + 提示词）"""
    base_dir = Path(os.environ.get("CONFIG_DIR", "config"))
    router_path = Path(
        os.environ.get("ASSISTANT_ROUTER_CONFIG", str(base_dir / "assistant_router.yaml"))
    )
    router = {}
    if router_path.exists():
        try:
            ...
            print(f"助理路由配置加载成功: {router_path}")
        except Exception as e:
            print(f"助理路由配置加载失败，使用默认配置: {e}")
    else:
        print(f"未找到助理路由配置文件，使用默认配置: {router_path}")

    return {
        "ROUTE_RULES": router.get("route_rules", {}),
        "SYSTEM_PROMPTS": router.get("system_prompts", {}),
        "LLM_ROUTING": router.get("llm_routing", {}),
        "CONTEXT": router.get("assistant_context", {}),
    }
...
def load_config(config_path: Optional[str] = None) -> Dict[str, Any]:
    ...
    # 助理路由配置
    config["ASSISTANT_ROUTER"] = _load_assistant_router_config(config_data)
