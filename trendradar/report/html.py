...
def _resolve_header_background_url() -> str:
    """返回头图背景 URL，优先内嵌 data URI，失败时回退到相对路径。"""
    ...
    try:
        encoded = base64.b64encode(header_path.read_bytes()).decode("ascii")
...
def _resolve_optional_asset_data_uri(asset_filename: str) -> Optional[str]:
    """读取 ``assets/<asset_filename>`` 并返回 data URI；不存在或失败则返回 None。"""
    project_root = Path(__file__).resolve().parents[2]
    asset_path = project_root / "assets" / asset_filename
    if not asset_path.exists():
        return None
    mime = _MIME_BY_SUFFIX.get(asset_path.suffix.lower(), "application/octet-stream")
...
def _search_bar_background_css_fragment(data_uri: str) -> str:
    """搜索框自定义背景：PNG 可为透明边框图；底色填在透明区域之下，不再叠白色渐变以免把透明抠坏。"""
    # 缩进与 .search-input 块内其它属性对齐（14 空格）
    return (
...
def _rss_item_background_extra_css(data_uri: str) -> str:
    """RSS 条目可选背景图（底色已在 CSS 中设为 #eff6ff）；订阅与新增共用。"""
    return (
        f"                background-image: url('{data_uri}');\n"
        "                background-repeat: no-repeat;\n"
        "                background-position: center;\n"
...
def render_html_content(
    ...
    *,
    ...
    header_bg_url = _resolve_header_background_url()
    search_bg_uri = None
    for _fname in ("search.png", "search_bg.png", "search_bg.jpg", "search_bg.jpeg", "search_bg.webp"):
        search_bg_uri = _resolve_optional_asset_data_uri(_fname)
        if search_bg_uri:
            break
    search_bg_css = _search_bar_background_css_fragment(search_bg_uri) if search_bg_uri else ""

    rss_item_bg_uri = None
    for _fname in (
        "rss_new_item.png",
        "rss_new_item_bg.png",
        "rss_new_item_bg.jpg",
        "rss_new_item_bg.jpeg",
        "rss_new_item_bg.webp",
    ):
        rss_item_bg_uri = _resolve_optional_asset_data_uri(_fname)
        if rss_item_bg_uri:
            break
    rss_item_bg_extra_css = (
        _rss_item_background_extra_css(rss_item_bg_uri) if rss_item_bg_uri else ""
    )
        ...
        <style>
            * { box-sizing: border-box; }
            ...
            .header {
                ...
                background-image: url('__HEADER_BG_URL__');
                background-position: center;
                background-repeat: no-repeat;
                background-size: cover;
                color: #f8fafc;
            ...
            .header-title {
                ...
                z-index: 2;
                color: #ffffff;
                -webkit-text-stroke: 0.35px rgba(15, 23, 42, 0.65);
            ...
            .header .info-label {
                ...
                margin-bottom: 4px;
                -webkit-text-stroke: 0.22px rgba(15, 23, 42, 0.55);
                paint-order: stroke fill;
                text-shadow:
            ...
            .header .info-value {
                ...
                font-size: 16px;
                color: #ffffff;
                -webkit-text-stroke: 0.3px rgba(15, 23, 42, 0.65);
                paint-order: stroke fill;
                text-shadow:
                    0 0 2px rgba(0, 0, 0, 0.95),
            ...
            .word-count.hot { color: #38bdf8; font-weight: 600; }
            .word-count.warm { color: #7dd3fc; font-weight: 600; }
            ...
            .rss-section-title {
                font-size: 18px;
                font-weight: 600;
                background: linear-gradient(135deg, #1d4ed8 0%, #0891b2 52%, #6366f1 100%);
...
__RSS_ITEM_BG_CSS__
            ...
            /* RSS 条目可选自定义背景图：assets/rss_new_item*.png（订阅与新增共用） */
            ...
            .tab-btn:hover { background: #e0f2fe; color: #38bdf8; }
            .tab-btn.active { background: #bae6fd; color: #0369a1; }
            .tab-count {
                font-size: 11px;
            ...
            .tab-btn.active .tab-count { background: rgba(255,255,255,0.55); color: #0369a1; }
...
__SEARCH_BG_CSS__
            ...
            .fab-btn {
                ...
                height: 40px;
                border-radius: 50%;
                background: linear-gradient(145deg, #38bdf8 0%, #7dd3fc 55%, #67e8f9 100%);
                color: white;
                border: none;
                cursor: pointer;
                font-size: 16px;
            ...
            .assistant-modal-close:hover { background: #cbd5e1; }
            .assistant-frame {
                flex: 1;
                width: 100%;
                border: none;
    ...
    html += """
        ...
        <div class="fab-bar">
            <button class="fab-btn" onclick="openAiAssistant()" title="打开 AI 助理">AI</button>
        ...
        <div class="assistant-modal" id="assistantModal" onclick="handleAssistantModalBackdrop(event)">
            <div class="assistant-modal-dialog">
                <div class="assistant-modal-header">
                    <div class="assistant-modal-title">AI 助理</div>
                    <button class="assistant-modal-close" type="button" onclick="closeAiAssistant()" title="关闭">×</button>
                </div>
                <iframe class="assistant-frame" id="assistantFrame" title="AI 助理聊天窗口"></iframe>
            </div>
        </div>
        ...
        <script>
            ...
            function openAiAssistant() {
                var assistantUrl = '/assistant';
                if (window.location.protocol === 'file:') {
                    assistantUrl = 'http://127.0.0.1:8765/assistant';
                }
                var modal = document.getElementById('assistantModal');
                var frame = document.getElementById('assistantFrame');
                if (!modal || !frame) return;
                if (frame.getAttribute('src') !== assistantUrl) {
                    frame.setAttribute('src', assistantUrl);
                }
            ...
            function handleAssistantModalBackdrop(event) {
    ...
    return (
        html.replace("__HEADER_BG_URL__", header_bg_url)
        .replace("__SEARCH_BG_CSS__", search_bg_css)
        .replace("__RSS_ITEM_BG_CSS__", rss_item_bg_extra_css)
    )
